...
def query_property(g: Graph, property_uri: URIRef):
    property_uri = property_uri.n3()[1:-1]
    qry = """SELECT DISTINCT ?object 
                WHERE {
                ?s <?property> ?object . FILTER(isLiteral(?object)) 
            }""".replace("?property", property_uri)
    results = g.query(qry, initBindings={'property': property_uri})
    if not results:
        return None

    # Dictionary to store properties and their numerical values
    values = set([])

    # Iterate through the results
    for row in results:
        obj = row.object
        if obj.datatype in (XSD.decimal, XSD.float, XSD.integer):   # isinstance(obj, rdflib.Literal) and 
            values.add(float(obj))
    if values:
        return {'max_min': max(values) - min(values)}


class MyOWLOntology():
    def __init__(self, ont_file:str, pr: str = None):
        ...
        self.object_relations = set([]) # set: object type relations
        self.num_relations_max_min = {} # dict: the max and min of the numerical values in the range of a relation
        ...
        ##################   Hao   ##################
        datatype_properties = set(self.o.subjects(RDF.type, OWL.DatatypeProperty))
        if OWL.topDataProperty in datatype_properties:
            datatype_properties.remove(OWL.topDataProperty)
        for op in datatype_properties:
            self.relations[op.n3()[1:-1]] = OWLRelation(op, self) 
        self._relations = [rel.p for rel in self.relations.values()]
        print('Datatype Properties read', datatype_properties)
        
        # numerical_properties = {prop for prop in datatype_properties if any(self.o.triples((prop, RDFS.range, dtype)) for dtype in numerical_datatypes)}
        numerical_properties = set([])
        for p in datatype_properties:
            max_min = query_property(self.o, p)
            if max_min is not None:
                numerical_properties.add(p)
                self.num_relations_max_min[p] = max_min
        print('Numerical Properties read', self.num_relations_max_min)
        ##################   Hao   ##################


        self.object_relations = set(self.o.subjects(RDF.type, OWL.ObjectProperty)) 
        print("object property", self.object_relations)
        if OWL.topObjectProperty in self.object_relations:
            self.object_relations.remove(OWL.topObjectProperty)
        for op in self.object_relations:
            self.relations[op.n3()[1:-1]] = OWLRelation(op, self)
        # self._relations = [rel.p for rel in self.relations.values()] # Hao
        self._relations += [rel.p for rel in self.relations.values()]
        print('Object Properties read')
        ...
        print('Classes read')
        for c1 in list(self.concepts.values())[:100]:
            for c2 in list(self.concepts.values())[:100]:
        ...
        # indivs.update({o for _, _, o in self.o.triples((None, None, None))})
        indivs = list(indivs)
        for ind in indivs:
        ...
        for ind1 in sorted(list(self.individuals.values()))[:20]:
            for ind2 in sorted(list(self.individuals.values()))[:20]:
                if ind1 == ind2: 
                    continue
                print(str(ind1), 'and', str(ind2), 'has similarity ', ind1.similarity(ind2))
    ...
    def get_individual_OWL_link(self, ind: MyOWLIndividual) -> list:
        ...
        for r in self.relations.values():
            ...
            # print(ind, "-----neighbors---", neighs)

            for neigh in neighs:
                if not isinstance(neigh, rdflib.BNode):
                    aux1 = MyOWLIndividual(neigh, self)
                    if aux1 is not None:
                        link = OWLLink(r, aux1)     # OWLLink(OWLRelation, MyOWLLogicalEntity)  # Hao
                        owl_links.add(link)
                    else:
        ...
        # print(ind, "-----links---", [(l.relation.p, l.destiny.uri) for l in owl_links])
        return owl_links
...
class OWLLink():
    ...
    def similarity(self, a: OWLLink, concept_a: MyOWLLogicalEntity, concept_b: MyOWLLogicalEntity):
        ...
        try:
            ...
            # sim_tax_des = self.destiny.taxonomic_similarity(a.destiny) # Hao
            # sim_exp = 1 
            # sim = sim_tax_rel * sim_tax_des * sim_exp

            ##################  Hao  ##################
            
            # print("pair: ", self.relation, self.destiny.uri," -- ", a.relation, a.destiny.uri)
            sim_tax_des = self.destiny.taxonomic_similarity(a.destiny) if self.relation.p in concept_a.o.object_relations else 1.0  # Hao
            sim_val = 1.0 
            if self.relation.p != a.relation.p:   # if properties of pairs(x,y) are not the same
                sim_val *= 0.1 
            else:
                if self.relation.p in concept_a.o.num_relations_max_min.keys(): # if the property is numerical property
                    tmp_sim = abs(float(self.destiny.ind) - float(a.destiny.ind))
                    if tmp_sim == 0:
                        sim_val *= 1
                    else:
                        sim_val *= 1 - tmp_sim / concept_a.o.num_relations_max_min[self.relation.p]['max_min']
                else:   # categorical properties: obejct_type union literals 
                    sim_val *= 1 if self.destiny.ind == a.destiny.ind else 0.15
            # print("similarity: ", sim_val)
            sim = sim_tax_rel * sim_tax_des * sim_val 
            ##################  Hao  ##################
...
class MyOWLIndividual(MyOWLLogicalEntity):
    def __init__(self, a: rdflib.URIRef, onto: MyOWLOntology):
        self.o = onto
        self.uri = a.n3()[1:-1] if isinstance(a, rdflib.URIRef) else str(a)
        self.neighbors = None
        self.ind = a
    
    def __lt__(self, other):
        """Define less-than comparison on self.ind."""
        if isinstance(other, MyOWLIndividual):
            return str(self.ind) < str(other.ind)
        return NotImplemented

    def __eq__(self, other):
        """Define equality comparison on self.ind."""
        if isinstance(other, MyOWLIndividual):
            return str(self.ind) == str(other.ind)
        return NotImplemented
    ...
    def similarity_neighbors(self, c: MyOWLIndividual):
        ...
        # print("neighbors", self.neighbors, " -- ", c.neighbors)
        sim = 0.0 
        if not self.neighbors or not c.neighbors:
            return sim
        try:
    ...
    def similarity(self, c):
        ...
        #     sim = self.on_sim(c)
        #     return sim
        # if isinstance(c, OWLConcept) or isinstance(c, MyOWLIndividual):
...
class BipartiteGraphMatching():
    ...
    def matching(self, a: list, b: list, orig: MyOWLLogicalEntity, des: MyOWLLogicalEntity):
        ...
        self.assignment = hungarn.execute()
        # print()
        # print([(l.relation.p, l.destiny.uri) for l in a])
        # print([(l.relation.p, l.destiny.uri) for l in b])
        # print(self.assignment)
        # print(1-self.cost_matrix)
